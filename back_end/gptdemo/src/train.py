...
def train():
    ...
    never_split = ["[speaker1]", "[speaker2]", "[BOS]", "[SEP]", "[EOS]", "[CLS]", "[neutral]", "[happiness]", "[sadness]", "[anger]", "[disgust]"]
    if args.pretrained:
        tokenizer = tokenizer_class.from_pretrained(args.model_checkpoint, do_lower_case=True, never_split=never_split)
        tokenizer.additional_special_tokens = never_split
